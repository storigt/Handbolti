  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Review the match details before going live.</p>
      </div>

      <Card className="p-4 space-y-3">
        <h3 className="font-medium text-gray-800">Match</h3>
        <div className="flex items-center justify-center gap-4 py-3">
          <div className="text-center">
            <p className="font-bold text-lg">{trackedTeam.short_name ?? trackedTeam.name}</p>
            {isHome && <p className="text-xs text-gray-400">Home</p>}
          </div>
      ...
      {isLoading ? (
        ...
        <Card className="p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="font-medium text-gray-800">Roster</h3>
            <span className="text-sm text-gray-500">{roster.length} players</span>
          </div>

          {starters.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Starting 7</p>
              <div className="flex flex-wrap gap-2">
                {starters.map(r => (
                  <span key={r.id} className="flex items-center gap-1 bg-blue-50 text-blue-800 text-sm px-2 py-1 rounded-full">
                    ...
                    {r.player.position === 'goalkeeper' && <span className="text-xs text-purple-600">GK</span>}
                  </span>
                ))}
              </div>
            </div>
          ...
          {bench.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Bench</p>
              <div className="flex flex-wrap gap-2">
                {bench.map(r => (
      ...
      <div className="flex justify-between">
        <Button variant="secondary" onClick={onBack}>← Edit roster</Button>
        <Button
